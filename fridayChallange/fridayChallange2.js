...
function nameSurname(name2, surname2) {
  const checkCapitalName =
    name2.slice(0, 1) == name2.slice(0, 1).toUpperCase() &&
    surname2.slice(0, 1) == surname2.slice(0, 1).toUpperCase()
      ? `${name2} ${surname2}`
      : "Vardas ir pavarde turi buti didziosionis!";
...
function createRandomArray(from, to, limit) {
  ...
  for (let i = 0; i < limit; i++) {
    randomNumbersArray += characters.charAt(
...
const arrayNew = [];
for (let i = 0; i < 100; i++) {
  if (randomArray[i] > 5000) {
    arrayNew.push(randomArray[i]);
  }
}
